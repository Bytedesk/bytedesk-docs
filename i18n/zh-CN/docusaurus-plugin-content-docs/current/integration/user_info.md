...
本页中的“用户信息对接”，主要指业务系统中的登录会员、注册用户，在进入客服会话时如何映射为访客侧身份信息。

它和后台权限体系中的“注册用户 User”不是同一个概念：

- 后台注册用户对应 `UserEntity`
- 会话访客对应 `VisitorEntity`

请重点区分：

- 业务系统中的已登录会员，可以在聊天时通过 `visitorUid`、`nickname`、`avatar` 映射为访客身份
- 这不会自动创建后台 `UserEntity`
- 这也不会自动赋予管理员、客服、主管等权限
- 只有显式创建的系统注册用户，才可以登录后台并被授予角色权限

如果您的目标是：

- 让商城会员进入客服会话并被识别：看本页
- 创建后台管理员、客服账号并配置权限：请看用户、角色、权限相关后台文档

本页面介绍如何将业务系统中的登录用户、注册会员信息映射到微语客服系统中的访客身份，以实现会话识别和个性化服务。
...
这里的“用户信息对接”更准确地说，是“将业务用户映射为访客会话身份”。

## 对接方法

微语客服系统支持通过配置参数传入用户信息。在初始化客服组件时，通过`chatConfig`配置项传入会话侧身份信息。

这些字段最终主要作用于访客身份展示，而不是后台账号授权。
...
chatConfig: {
  ...
  sid: 'df_wg_uid',         // 会话ID
    ...
    // 会话身份参数 - 这些字段会映射到访客身份
    visitorUid: 'visitor_001',       // 访客唯一ID，建议使用您业务系统中的用户ID
  nickname: '访客小明',     // 用户昵称
...
- 即使业务用户已经在您的电商系统中登录，这里传递到客服链接中的仍然是访客侧身份参数
- 这些参数用于会话识别，不等同于后台登录账号
