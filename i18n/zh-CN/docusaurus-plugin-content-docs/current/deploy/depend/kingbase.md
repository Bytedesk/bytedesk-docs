...
`DB_MODE` 选项值说明（用于 Kingbase 容器启动参数 `-e DB_MODE=...` 或 `.env` 中的 `KINGBASE_DB_MODE`）：

- `mysql`：MySQL 兼容模式（推荐当前微语默认使用）
...
- 这两种方式与环境变量 `DB_MODE/DB_USER/DB_PASSWORD` 无直接关系；仅依赖容器环境变量时不会自动创建业务库。
- 在本仓库 `deploy/docker/start.sh` 使用 `kingbase9` 启动时，脚本会在中间件启动后自动检查并创建 `KINGBASE_DATABASE`（默认 `bytedesk`）。
...
- 容器日志反复出现 “sudo: pam_open_session: Permission denied / policy plugin failed session initialization”
  ...
  1) 持久化（重建容器也生效）：基于官方镜像自建一个自定义镜像，在 Dockerfile 中添加 `echo "Defaults !pam_session" >> /etc/sudoers`，或解注释 `/etc/sudoers` 末尾的 `#includedir /etc/sudoers.d` 并向该目录投放禁用项。
