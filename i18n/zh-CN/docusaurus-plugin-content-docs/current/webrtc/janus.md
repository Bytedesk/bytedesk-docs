...
- GitHub 仓库：[https://github.com/Bytedesk/bytedesk-janus](https://github.com/Bytedesk/bytedesk-janus)
- Janus Docker 镜像：[https://hub.docker.com/r/bytedesk/janus](https://hub.docker.com/r/bytedesk/janus)

:::

## Janus 在微语音视频客服系统中的作用

在微语音视频客服系统中，Janus 不是单独暴露给业务用户的“前台服务”，而是位于访客端、客服端与微语服务端之间的实时媒体基础设施。它主要负责承接 WebRTC 媒体协商结果，并在需要时承担音视频转发、房间管理、媒体会话保持与扩展能力接入。

### Janus 承担的核心职责

- **媒体接入层**：承接访客端与客服端发起的 WebRTC 音视频流。
- **SFU/网关能力**：在多人会话、旁路录制、扩展插件场景中，作为媒体路由与转发节点。
- **与信令层解耦**：微语服务器负责会话、鉴权、排队、分配坐席、状态同步，Janus 负责音视频媒体面处理。
- **与 Coturn 协作**：网络复杂或 NAT 穿透失败时，通过 Coturn 提供 relay 候选，提高接通率。
- **能力接入基础设施**：为微语服务器内部的录制、质检、监控、音视频分析等模块提供稳定的媒体接入与分发基础。

### 在客服业务中的实际价值

- 访客发起视频客服或语音客服时，Janus 负责把浏览器或 App 的媒体流稳定接入到服务链路中。
- 坐席接听后，Janus 可以作为媒体交换节点，避免复杂网络下完全依赖终端直连。
- 当后续需要引入录制、巡检、旁听、AI 分析、多方协作时，Janus 更适合作为统一的媒体接入与分发层。
- 对微语服务端而言，这种拆分使业务系统可以聚焦在客服流程，而把低时延音视频处理交给专门的 WebRTC 基础设施。

## 微语音视频通话架构

下面的架构描述了访客端、客服端、Coturn、Janus 与微语服务器之间在一次音视频客服会话中的典型协作关系。
...
flowchart LR
    ...
    subgraph bytedesk[微语服务器]
        signaling[鉴权 排队 路由 信令 会话状态]
        biz[录制 质检 AI 分析 CRM]
    end

    visitor -->|登录 发起呼叫| signaling
    agent -->|登录 就绪 接听| signaling
    signaling -->|分配坐席 下发会话信息| agent
    signaling -->|返回房间与协商参数| visitor
    visitor -. ICE Candidate/STUN/TURN .-> coturn
    agent -. ICE Candidate/STUN/TURN .-> coturn
    visitor -->|Publish/Subscribe 媒体流| janus
    agent -->|Publish/Subscribe 媒体流| janus
    signaling <--> |信令控制 SDP/ICE/状态同步| janus
    janus -->|媒体流接入/事件回传| biz
```

### 音视频呼叫时序图

下面的时序图展示了一次典型的微语音视频客服通话，是如何在访客端、微语服务器、客服端、Coturn 与 Janus 之间建立起来的。

```mermaid
sequenceDiagram
    autonumber
    participant visitor as 访客端
    participant bytedesk as 微语服务器
    participant agent as 客服端
    participant coturn as Coturn
    participant janus as Janus

    visitor->>bytedesk: 发起音视频咨询请求
    bytedesk-->>visitor: 返回排队状态与会话信息
    bytedesk->>agent: 推送来电与接听请求
    agent-->>bytedesk: 坐席接受接听
    bytedesk->>janus: 创建或绑定媒体会话
    bytedesk-->>visitor: 下发 Janus 接入参数
    bytedesk-->>agent: 下发 Janus 接入参数

    par ICE 候选收集
        visitor->>coturn: STUN/TURN 探测并收集 candidate
        agent->>coturn: STUN/TURN 探测并收集 candidate
    end

    visitor->>bytedesk: 上报 SDP Offer / ICE Candidate
    agent->>bytedesk: 上报 SDP Answer / ICE Candidate
    bytedesk->>janus: 转发协商参数并驱动建链
    janus-->>bytedesk: 返回协商结果与会话状态
    bytedesk-->>visitor: 下发协商结果
    bytedesk-->>agent: 下发协商结果

    visitor->>janus: 建立 WebRTC 媒体连接
    agent->>janus: 建立 WebRTC 媒体连接
    note over visitor,agent: 网络较好时走更短媒体路径\n网络复杂时由 Coturn 提供 relay 兜底

    janus-->>visitor: 下行音视频流
    janus-->>agent: 下行音视频流
    janus->>bytedesk: 上报接通、断开、异常等状态
    bytedesk-->>visitor: 同步会话状态
    bytedesk-->>agent: 同步会话状态
```

### 典型呼叫流程

1. 访客端向微语服务器发起音视频客服请求。
2. 微语服务器完成鉴权、排队、路由与坐席分配，并通知客服端接听。
3. 微语服务器向双方下发会话标识、Janus 接入信息和必要的信令参数。
4. 访客端与客服端分别通过 STUN/TURN 收集 ICE 候选，必要时借助 Coturn 获取 relay 地址。
5. 双方将 SDP/ICE 等协商信息通过微语服务器或与 Janus 的信令控制链路完成交换。
6. Janus 建立媒体会话后承接音视频流转发，Coturn 在无法直连时负责 NAT 穿透与中继兜底。
7. 会话过程中的状态变更，例如接通、挂断、转接、超时、录制等，仍由微语服务器统一管理。

### 架构拆分的意义

- **微语服务器**：负责业务面，包括账号、组织、排队、分配、消息、工单、会话生命周期，以及录制、质检、AI 分析、CRM 等内部业务模块。
- **Janus**：负责媒体面，包括 WebRTC 接入、媒体流管理、转发、会话承载。
- **Coturn**：负责网络连通面，包括 STUN 地址发现与 TURN 中继兜底。

这种分层方式有利于把“客服业务逻辑”和“实时音视频基础设施”解耦，既方便业务快速演进，也更利于后续容量扩展、弱网优化和多端接入。
...
- 官方说明（英文）：[https://github.com/meetecho/janus-gateway?tab=readme-ov-file](https://github.com/meetecho/janus-gateway?tab=readme-ov-file)
...
- 在宿主机创建 `conf/` 目录，放入需要覆盖的 Janus 配置（可从官方仓库的 `conf/` 目录拷贝并按需修改）。常用文件：`janus.jcfg`、`janus.transport.http.jcfg`、`janus.transport.websockets.jcfg`，以及可选的 `janus.eventhandler.sampleevh.jcfg`。
...
services:
    janus-gateway:
        ...
        # 推荐：仅启动 Janus（与项目 compose 保持一致）
        command: ["/usr/local/bin/janus", "-F", "/usr/local/etc/janus"]
        # 可选：镜像内若包含并启用 Nginx 时，再改为 nginx + janus 启动方式
...
- [Nginx 安装](../deploy/depend/nginx.md)
