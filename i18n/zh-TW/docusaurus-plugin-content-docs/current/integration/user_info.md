...
本頁主要說明：業務系統中的登入會員、註冊使用者，如何在進入客服會話時映射為訪客身份。

這和後台權限體系中的註冊使用者不是同一件事：

- 後台註冊使用者對應 `UserEntity`
- 會話訪客對應 `VisitorEntity`

請重點區分：

- 業務系統中的登入會員，可以透過 `visitorUid`、`nickname`、`avatar` 映射為訪客身份
- 這不會自動建立後台 `UserEntity`
- 這也不會自動賦予管理員、客服、主管等權限
- 只有顯式建立的系統註冊使用者，才可以登入後台並被授予角色權限
...
更精確地說，本頁是在說明「將業務使用者映射為訪客會話身份」。

## 對接方法

微語客服系統支援透過配置參數傳入身份資訊。在初始化客服元件時，透過 `chatConfig` 傳入訪客側身份欄位。
...
chatConfig: {
  ...
  sid: 'df_wg_uid',         // 會話ID
    ...
    // 會話身份參數 - 這些欄位會映射為訪客身份
    visitorUid: 'visitor_001',       // 訪客唯一ID，建議使用您業務系統中的使用者ID
  nickname: '訪客小明',     // 使用者暱稱
