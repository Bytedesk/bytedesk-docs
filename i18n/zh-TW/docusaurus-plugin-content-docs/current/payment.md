...
| 模組 | 價格 | 說明 |
| --- | --- | --- |
| 管理後台源碼 | ¥59,800 | 前端／React |
| 客服工作台源碼 | ¥59,800 | 前端／React，支援 Web 和 PC 端 |
| 知識庫幫助中心編輯器源碼 | ¥19,800 | 前端／React |
| 工單流程／工作流／表單低代碼源碼 | ¥39,800 | 前端／React |
| 數據大屏模組源碼 | ¥19,800 | 前端／React |
| 培訓考試模組源碼 | ¥29,800 | 前端／React，開發中 |
| 手機 App 客服端源碼 | ¥59,800 | Android／iOS／Web／Mac／Windows，Flutter |
| 小程式客服端源碼 | ¥59,800 | 微信小程式等，開發中 |
| 訪客端 H5 聊天源碼 | ¥39,800 | 前端／React，含線上客服與智慧客服聊天對話 |
| 伺服器高級功能源碼 | ¥59,800 | 後端／Java，[高級功能](#高級功能) |
...
| 呼叫中心客服工作台源碼 | ¥59,800 | 前端／React，支援 Web 和 PC 端 |
| 呼叫中心伺服器高級源碼 | ¥99,800 | 後端／Java |
| 呼叫中心訪客端源碼 | ¥19,800 | 前端／React，主要用於網頁版測試對話 |

### 語音模組源碼定價

| 模組 | 價格 | 說明 |
| --- | --- | --- |
| 音視頻客服源碼 | ¥99,800 | 前後端／React + Java，因以模組形式整合到管理後台、客服工作台中，以及獨立的 H5 訪客音視頻模組，所以僅在已購買管理後台源碼與客服工作台源碼的情況下才支援購買 |
| 視訊會議源碼 | ¥199,800 | 含伺服器高級功能源碼（Java）、Web／桌面客戶端（React／Electron）、手機端 App（Flutter），不含管理後台源碼，支援視訊會議、遠端教學、視訊面試、螢幕共享、白板、視訊錄製等功能 |
