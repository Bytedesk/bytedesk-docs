...
## 4. VIP-restricted authorities and levels

To support features that are available only for paid organizations, the permission model includes these fields:

- Organization side: `vip` (is paid member) + `vipLevel` (membership tier)
- Authority side: `vipOnly` (whether this authority is VIP-only) + `vipLevel` (minimum required VIP tier)

Interpretation:

- `vipOnly = false`: no VIP restriction
- `vipOnly = true` and authority `vipLevel = 1`: paid organizations only
- `vipOnly = true` and authority `vipLevel > 1`: requires the corresponding minimum VIP tier

Recommendation:

- First mark premium capabilities as `vipOnly`
- Then split access further with `vipLevel` as you introduce multiple paid tiers
...
### 5.3 Admin Console behavior (Frontend)
...
### 6.3 Configure role authorities
...
### 7.1 Why can a user still operate after the role expired?
...
### 7.2 What’s the difference between “Permanent” and “Field not provided”?
...
### 7.3 How to read organization VIP level vs authority VIP level?

- Organization `vipLevel` means the current membership tier of the organization.
- Authority `vipLevel` means the minimum tier required to use that authority.

When `vipOnly=true`, the authority should only be granted or displayed if organization tier is greater than or equal to the authority tier.
